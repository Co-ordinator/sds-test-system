...
- **[Contributor Guide](CONTRIBUTING.md)** - Safe development, testing, migration, and documentation workflow
- **[Changelog](docs/CHANGELOG.md)** - User-impacting changes and deployment notes
## Test Credentials
...
### PDF and Certificate Assets
PDF reports and certificates are generated by the backend, so the backend must have local access to these image files:

- `backend/assets/letterhead.png`
- `backend/assets/siyinqaba.png`
- `backend/assets/watermark.png`

These files are now part of the backend source. When `npm run build --prefix backend` is used, the build script also copies them into `backend/dist/assets`. Do not rely only on `frontend/public` for hosted PDF/certificate generation, because the Node.js backend cannot read browser static files unless they are also deployed with the backend or configured through `PDF_LETTERHEAD_PATH` and `PDF_WATERMARK_PATH`.

## Contact
For support or questions, contact the SDS project team at notificationsdatamatics@gmail.com
