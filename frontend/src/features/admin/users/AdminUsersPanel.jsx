...
import { Search, RefreshCw, Download, Edit2, Trash2, Eye, X, Plus, Shield, Mail, ChevronRight, CheckCircle2, XCircle, UserX, Filter, Upload } from 'lucide-react';
...
import { adminUserDisplayName } from '../../../utils/adminUserDisplay';
...
const AdminUsersPanel = ({ institutions = [] }) => {
  ...
  const [isImportingAdmins, setIsImportingAdmins] = useState(false);
  ...
  const downloadSystemAdminTemplate = () => {
    const csvContent = `first_name,last_name,email,organization
Sipho,Dlamini,sipho.dlamini@example.org,Ministry of Labour and Social Security
Nomcebo,Mkhonza,nomcebo.mkhonza@example.org,Ministry of Education and Training`;
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', 'system_admin_import_template.csv');
  ...
  const toolbar = (
    <>
      ...
      <PermissionGate permission="users.create">
        <button
          type="button"
          onClick={downloadSystemAdminTemplate}
          className="flex items-center gap-1 px-3 py-1.5 border rounded-md text-xs bg-white"
        ...
        <label className="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold border cursor-pointer" style={{ borderColor: GOV.border, color: GOV.text }}>
          ...
          <input
            ...
            onChange={async (e) => {
              ...
              if (!file) return;
              try {
                setIsImportingAdmins(true);
                const text = await file.text();
                const report = await adminService.importSystemAdmins(text);
                showToast(`Imported ${report?.createdCount || 0} system admin(s)`);
                if (report?.failedCount) {
                  showToast(`${report.failedCount} row(s) failed. Check backend response logs.`, 'error');
                }
                load();
              } catch (err) {
                showToast(err.uiMessage || 'System admin import failed', 'error');
              } finally {
                setIsImportingAdmins(false);
              }
            }}
          />
        </label>
