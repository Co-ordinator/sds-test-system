 ...
 *
 * Props:
 *   value        — display string (workplace name)
 *   institutionId — UUID of the matched institution (null for free text)
 ...
 */
export default function WorkplaceSearchInput({
  value = '',
  institutionId = null,
  onChange,
  ...
  const handleInputChange = (e) => {
    ...
    setQuery(val);
    setOpen(true);
    // Notify parent: free text, no institution ID yet
    onChange(val, null);
    // Debounce API call
    ...
    <div className="relative" ref={containerRef}>
      ...
      {institutionId && (
        <p className={`mt-1 flex items-center gap-1 ${TYPO.hint}`} style={{ color: '#059669' }}>
          ...
          Matched to a registered institution
        </p>
      )}
      ...
      {open && (
        ...
        >
          ...
          {!loading && results.length === 0 && query.trim().length > 0 && (
            <li className={`px-3 py-2 ${TYPO.hint}`} style={{ color: GOV.textHint }}>
              No registered institution found — your entry will be saved as typed.
            </li>
          )}
