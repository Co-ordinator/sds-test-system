...
import { Clock, Loader2, User, LogOut, Eye, X, FileText, Award } from 'lucide-react';
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div className="border rounded-md p-3" style={{ borderColor: GOV.border }}>
                      ...
                      <p className="text-lg font-bold mt-1 font-mono" style={{ color: GOV.text }}>{getHollandDisplayCode(selectedAssessment)}</p>
                    </div>
                    <div className="border rounded-md p-3" style={{ borderColor: GOV.border }}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-8">
        ...
        {inProgress && (
          <div className="bg-white border rounded-lg shadow-sm p-10 text-center" style={{ borderColor: GOV.border }}>
            ...
            <button
              type="button"
              onClick={() => navigate('/questionnaire', { state: { resumeAssessment: true } })}
              ...
              <tbody>
                ...
                {completed.map((a) => (
                  <tr key={a.id} className="align-middle border-b" style={{ borderColor: GOV.borderLight }}>
                    ...
                    <td className="py-3 px-4 font-medium" style={{ color: GOV.text }}>{getHollandDisplayCode(a)}</td>
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {certificates.find(c => c.assessmentId === a.id) ? (
                          ...
                          >
                            ...
                            {downloadingCert === a.id ? 'Downloading...' : 'Certificate'}
                          </button>
                        ) : (
                          <button
                            type="button"
                            onClick={() => handleGenerateCertificate(a.id)}
                            disabled={generatingCert === a.id || downloadingCert === a.id}
                            className="flex items-center gap-1 px-3 py-1 rounded text-xs font-semibold disabled:opacity-50 transition-all"
                            style={{ backgroundColor: '#d97706', color: '#fff', border: 'none' }}
                          >
                            {generatingCert === a.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Award className="w-3 h-3" />}
                            {generatingCert === a.id ? 'Generating...' : 'Generate Certificate'}
                          </button>
                        )}
                      </div>
                    </td>
